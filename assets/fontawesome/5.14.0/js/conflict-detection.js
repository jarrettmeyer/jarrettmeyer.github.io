...
(function (global, factory) {
    ...
    if (conflictsCount > 0) {
      ...
      for (const _key in nodesTested.conflict) {
        ...
        data[_key] = {
          ...
          "innerText excerpt": item.textContent && item.textContent !== "" ? item.textContent.slice(0, 200) + "..." : "(empty)",
        };
      }
    ...
    if (noConflictsCount > 0) {
      ...
      for (const _key2 in nodesTested.noConflict) {
        ...
        _data[_key2] = {
          ...
          "innerText excerpt": _item.textContent && _item.textContent !== "" ? _item.textContent.slice(0, 200) + "..." : "(empty)",
        };
      }
    ...
    if (timeOutCount > 0) {
      ...
      for (const _key3 in timedOutTests) {
        ...
        _data2[_key3] = {
          ...
          "innerText excerpt": _item2.textContent && _item2.textContent !== "" ? _item2.textContent.slice(0, 200) + "..." : "(empty)",
        };
      }
  ...
  function detectWebfontConflicts() {
    ...
    const styleTags = Array.from(DOCUMENT.getElementsByTagName("style")).filter(function (t) {
      ...
      if (WINDOW.FontAwesomeConfig && t.textContent.match(new RegExp("svg:not\\(:root\\)\\.".concat(WINDOW.FontAwesomeConfig.replacementClass)))) {
        return false;
      }
    ...
    function runDiag(scriptOrLinkTag, md5) {
      ...
      diagScript.textContent = "(".concat(diagScriptFun.toString(), ")('").concat(nodeUnderTestId, "', '").concat(testIconId || "foo", "', '").concat(md5, "', '").concat(parentOrigin, "');");
  ...
  function detectSvgConflicts(currentScript) {
    ...
    const _loop = function _loop(scriptIdx) {
      ...
      diagScript.textContent = "(".concat(diagScriptFun.toString(), ")('").concat(nodeUnderTestId, "', '").concat(md5ForScript, "', '").concat(parentOrigin, "');");
