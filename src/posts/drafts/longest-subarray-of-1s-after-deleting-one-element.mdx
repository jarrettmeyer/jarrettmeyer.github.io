...
<Alert type="info">
  This problem is very similar to [Max Consecutive Ones
  III](/leetcode/max-consecutive-ones-iii/), where you can flip at most `k`
  zeros to ones. Both problems can be solved using a sliding window approach
  that allows at most one zero (or one deletion) in the window.
</Alert>
